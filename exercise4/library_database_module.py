...
def get_book_detail(book_id):
    connection = sqlite3.connect("library.db")
    cursor = connection.cursor()
    status=cursor.execute("SELECT status FROM Books WHERE book_id=?", (book_id,))
...
def get_reservation_by_title(title):
    connection = sqlite3.connect("library.db")
    cursor = connection.cursor()
    cursor.execute("SELECT * FROM Reservations WHERE title=?", (title,))
    reservations = cursor.fetchall()
    connection.close()
    return reservations
...
def update_book_status(book_id, title, author, isbn, status, user_id):
    ...
    if status == "On reservation":
        ...
        cursor.execute("UPDATE Books SET title=?, author=?, isbn=?, status=? WHERE book_id=?", (title, author, isbn, status, book_id))
        cursor.execute("DELETE FROM Reservations WHERE bood_id=?", (book_id,))
    connection.commit()
    connection.close()
    return cursor.rowcount > 0
